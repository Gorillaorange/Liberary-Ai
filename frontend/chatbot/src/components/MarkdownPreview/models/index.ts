  ...
  
{
    ...
    transformStreamValue(readValue, textDecoder) {
      console.log("====== 本地模型处理流开始 ======");
      let rawContent = '';
      
      // 解码数据
      if (readValue instanceof Uint8Array) {
        rawContent = textDecoder.decode(readValue, { stream: true });
      } else if (typeof readValue === 'string') {
        rawContent = readValue;
      } else {
        console.warn("无法识别的数据类型:", typeof readValue);
        return { content: '' };
      }
      
      console.log("接收原始数据:", rawContent.slice(0, 100) + (rawContent.length > 100 ? '...' : ''));
      ...
      for (const line of lines) {
        ...
        if (line.startsWith('data:')) {
          let dataContent = line.replace('data:', '').trim();
          ...
          try {
            ...
            // 如果有明确的完成标志
            if (jsonData.done || jsonData.complete || jsonData.finished) {
              console.log("从JSON中检测到完成标志");
              return { done: true };
            }
            ...
            if (jsonData.type === "content" && jsonData.data) {
              ...
              // 无法从JSON中提取有效内容，将整个JSON转为字符串
              actualContent = JSON.stringify(jsonData);
            }
            
            combinedContent += actualContent;
          } catch (e) {
            // 不是JSON格式，直接使用文本内容
            combinedContent += dataContent;
          }
        } else if (line.includes("[DONE]") || line.includes("event: done")) {
          // 处理其他格式的完成标志
          console.log("检测到其他格式的完成标志");
          return { done: true };
        }
      }
      ...
      // 返回组合后的内容
      if (combinedContent) {
        console.log("返回处理后内容:", combinedContent.slice(0, 100) + (combinedContent.length > 100 ? '...' : ''));
        return { content: combinedContent };
      }
      
      // 如果没有内容但没有错误，返回空字符串而不是null
      return { content: '' };
    },
    async chatFetch(text, sessionId) {
      try {

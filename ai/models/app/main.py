...
# 移除未使用的导入 - from app.configs.prompts import RESPONSE_PROMPT
...
async def create_item(request: Request):
    ...
    async def generate_response():
        ...
        for output_ids in generated_ids:
            # 解码当前生成的token
            current_token = tokenizer.decode(output_ids[-1], skip_special_tokens=True)
            if current_token:
                # 处理响应
                think_content, answer_content = split_text(current_token)
...
@app.post("/aiforward895686727887224832/embeddings")
...
@app.post("/aiforward895686727887224832/search")
async def search_similar(request: SearchRequest):
    try:
        ...
        try:
            ...
            if hits:
                logger.info(f"从Milvus找到 {len(hits)} 个结果")
                return {"hits": hits}
        except Exception as e:
        ...
        hits = search_from_persistent_storage(query_embedding, request.limit)
        if hits:
            logger.info(f"从持久化存储找到 {len(hits)} 个结果")
            return {"hits": hits}
...
@app.get("/aiforward895686727887224832/health")

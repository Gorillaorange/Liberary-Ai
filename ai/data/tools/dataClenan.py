...
# ================== 核心清洗逻辑 ==================
def clean_local_jsonl(input_file, output_file, max_samples=50000):
    ...
    processed = 0
    saved = 0
    repair_log = defaultdict(int)  # 新增修复统计
    ...
    with open(input_path, 'r', encoding='utf-8', errors='replace') as fin, \
        ...
        for line_num, line in enumerate(fin, 1):
            try:
                ...
                # 进度显示
                if line_num % 100 == 0:
                    sys.stdout.write(f"\r[Progress] 处理: {line_num} | 有效: {saved}")
                    sys.stdout.flush()
    ...
    # 最终报告
    print(f"\n[Complete] 共处理 {processed} 条数据")
    print(f"[Stats] 规则咨询: {intent_counter['规则咨询']}")
    print(f"[Stats] 书籍推荐: {intent_counter['书籍推荐']}")
